<template>
  <div class="dev gap-1 bg-white">
    <h3 :class="`text-${props.titleSize || 'xl'}`">{{ props.title }}</h3>
    <JsonViewer v-if="json" :value="props.data" />
  </div>
</template>
<script setup>
...
const props = defineProps(['data', 'title', 'titleSize']);
</script>
<style>
.jv-code {
...
.jv-container .jv-toggle {
  background-image: url('https://trinity-inc.s3.ap-southeast-1.amazonaws.com/public/img/icon/json-icon.svg');
}
/*  */
.jv-container {
...
.jv-container .jv-toggle {
  background-image: url('_nuxt/assets/img/json-icon.svg');
